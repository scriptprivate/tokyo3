...
process_input_loop:
    ...
    syscall
    sw $v0, 0($s0)($t0)                 # Store input into array[i]
    addi $t0, $t0, 4                    # i++
...
process_sort_find_min_loop:
    ...
    lw $t3, 0($s0)($t2)                      # Load array[j] into $t3
    lw $t4, 0($s0)($t1)                      # Load array[min_index] into $t4
...
process_sort_swap:
    ...
    lw $t5, 0($s0)($t0)                     # temp = array[i]
    lw $t6, 0($s0)($t1)                     # Load array[min_index]
    sw $t6, 0($s0)($t0)                     # array[i] = array[min_index]
    sw $t5, 0($s0)($t1)                     # array[min_index] = temp
...
process_output_loop:
    ...
    lw $a0, 0($s0)($t0)                  # Load array[i] into $a0
